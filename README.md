...
<p>
  <a href="https://github.com/oculix-org/Apertix/releases"><img src="https://img.shields.io/badge/version-4.14.0--0-blue?style=flat-square&logo=github" alt="Version"></a>
  <a href="https://opencv.org/"><img src="https://img.shields.io/badge/OpenCV-4.14.0-red?style=flat-square&logo=opencv" alt="OpenCV"></a>
...
Apertix picks up from there. It compiles OpenCV 4.14.0 from the official sources, bindings and natives together, on GitHub Actions runners, and ships the result as a single jar. Add the dependency, call one method, use OpenCV.

| Project | Java bindings | Native binaries | Status |
|---|---|---|---|
| opencv/opencv | 4.14.0 | source only | upstream |
| openpnp/opencv | 4.3.0 | 4.9.0 | stopped February 2024 |
| Apertix | 4.14.0 | 4.14.0 | maintained here |
...
Versions up to `4.10.0-3` were published as `io.github.julienmerconsulting.apertix:opencv` and stay available on Maven Central under those coordinates. From `4.14.0-0` on, Apertix lives in the [oculix-org](https://github.com/oculix-org) organisation next to [Legerix](https://github.com/oculix-org/Legerix) and [Octachorix](https://github.com/oculix-org/Octachorix), and publishes under `io.github.oculix-org`.
...
|---|---|---|---|
| Windows | x86-64 | `win32-x86-64/opencv_java4140.dll` | official OpenCV Windows distribution |
| Windows | x86 | `win32-x86/opencv_java4140.dll` | official OpenCV Windows distribution |
| macOS | Intel | `darwin/libopencv_java4140.dylib` | compiled on `macos` Intel runner |
| macOS | Apple Silicon | `darwin-aarch64/libopencv_java4140.dylib` | compiled on `macos-14` |
| Linux | x86-64, glibc 2.35 or newer | `linux-x86-64/libopencv_java4140.so` | compiled on `ubuntu-latest` |
| Linux | x86-64, glibc 2.28 or newer | `linux-x86-64-legacy/libopencv_java4140.so` | compiled in `manylinux_2_28_x86_64` |
| Linux | aarch64, glibc 2.35 or newer | `linux-aarch64/libopencv_java4140.so` | compiled on `ubuntu-22.04-arm` |
| Linux | aarch64, glibc 2.28 or newer | `linux-aarch64-legacy/libopencv_java4140.so` | compiled in `manylinux_2_28_aarch64` |
| Linux | ARMv7 | `linux-arm/libopencv_java4140.so` | compiled on `ubuntu-22.04-arm` |
...
The repository holds no native binary. `git clone` gives you the loader, the tests and the upstream `opencv-4140.jar` the pom unpacks; the natives exist only in CI artifacts and in GitHub releases. Cloning is a few megabytes, not a few hundred.
...
The `nu.pattern` loader was written by Michael Ahlers at [Pattern Consulting](https://github.com/PatternConsulting/opencv) in 2014, and the commit history here starts with his. Jason von Nieda carried the packaging at [openpnp/opencv](https://github.com/openpnp/opencv) from 2.4.9 to 4.9.0. Apertix continues that work under its own name so that nobody mistakes it for an openpnp release.
